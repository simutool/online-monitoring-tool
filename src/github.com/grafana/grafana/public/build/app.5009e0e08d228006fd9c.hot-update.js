...
var InfluxQueryCtrl = /** @class */ (function (_super) {
    ...
    InfluxQueryCtrl.prototype.getFields = function () {
        ...
        this.datasource.metricFindQuery(fieldsQuery).then(function (results) {
            ...
            if (_this.templateSrv.variableExists("$Sensor")) {
                _this.templateSrv.getVarFromIndex()["Sensor"].query = sensStrings.join(',');
                _this.variableSrv.updateOptions(_this.templateSrv.getVarFromIndex()["Sensor"]);
            }
            else {
                variable = _this.variableSrv.createVariableFromModel({ type: "custom" });
                variable.name = "Sensor";
                variable.query = sensStrings.join(',');
                _this.variableSrv.updateOptions(variable);
                _this.variableSrv.addVariable(variable);
                console.log(_this.templateSrv.getVarFromIndex()["Sensor"]);
            }
            if (_this.templateSrv.variableExists("$Simulation")) {
                _this.templateSrv.getVarFromIndex()["Simulation"].query = simStrings.join(',');
                _this.variableSrv.updateOptions(_this.templateSrv.getVarFromIndex()["Simulation"]);
            }
            else {
                variable = _this.variableSrv.createVariableFromModel({ type: "custom" });
                ...
                _this.variableSrv.updateOptions(variable);
                _this.variableSrv.addVariable(variable);
                console.log(_this.templateSrv.getVarFromIndex()["Simulation"]);
            }
        });
    };
...
//# sourceMappingURL=app.5009e0e08d228006fd9c.hot-update.js.map